 ...
 */
class AdminRenderer implements RuntimeExtensionInterface
{
    /**
     * @var ResourceHelper
     */
     ...
     * @return string
     */
    public function generateResourcePath($resource, string $action = 'show', array $parameters = []): string
    {
     ...
     * @param bool              $asAttribute Whether to return as 'data-summary' html attribute
     *
     * @return string|null
     */
    public function generateSummaryPath(ResourceInterface $resource, bool $asAttribute = true): ?string
    {
        try {
            if ($asAttribute) {
                ...
                $routeName = $config->getRoute('summary');
                $route = $this->resourceHelper->findRoute($routeName);

                $parameters = $this->resourceHelper->buildParameters($route, $resource);
            ...
            return $this->resourceHelper->generateResourcePath($resource, 'summary');
        } catch (Throwable $e) {
        }
...
EOT;
     ...
     * @return array
     */
    private function getButtonOptions(string $action): array
    {
        if ($action === 'new') {
            return [
                'theme' => 'primary',
